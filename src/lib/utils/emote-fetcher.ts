...
class EmoteFetcher {
	...
	async fetchBetterTtvChannel(channelId: string): Promise<BetterTtvEmote[]> {
		return wetch<FetchBetterTTVChannelResponse>(
	...
	async fetchFrankerFaceZGlobal(): Promise<FrankerFaceZEmote[]> {
		return wetch<FetchFrankerFaceZGlobalResponse>(
			'/set/3',
			undefined,
	...
	async fetchFrankerFaceZChannel(channelId: string): Promise<FrankerFaceZEmote[]> {
		return wetch<FetchFrankerFaceZChannelResponse>(
			'/room/id/' + channelId,
			undefined,
			'https://api.frankerfacez.com/v1',
		).then((v) => {
